...
export class MarketTopLosers extends React.Component {

    render() {
        let { topLosers } = this.props
        let priceChangeConvert = this.props.priceChangeConvert;
        let companyNameConvert = this.props.companyNameConvert;

        return (
            <div className="Info-box" >
                <h3 className="Market-news-topic"><span className="star3">{"☆ "}</span>Top Gainers</h3>
...
const mapState = (state) => {
    return {
        topLosers: state.marketCompanies.marketTopLosers
    }
}
...
export default connect(mapState, mapDispatch)(MarketTopLosers)